...
class WebhookController < ApplicationController
  ...
  def create_response_text(from_mid, user_text)
    ...
    ActiveRecord::Base.transaction do
      ...
      if message_text == MasterQuestion.find(3)
        response_text = MasterQuestion.find(4)
      elsif message_text == MasterQuestion.find(4)
        response_text = MasterQuestion.find(5)
      elsif message_text == MasterQuestion.find(6) 
        # そろそろお腹すいた？？に対する返信
        if user_text == "はい"
          response_text = "どんな気分"
        else
